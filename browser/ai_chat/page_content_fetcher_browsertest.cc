  ...
  void FetchPageContent(const std::string& expected_text,
    ...
    ai_chat::FetchPageContent(
        ...
        base::BindLambdaForTesting(
            [&run_loop, &expected_text, &expected_is_video](std::string text,
                                                            bool is_video) {
...
IN_PROC_BROWSER_TEST_F(PageContentFetcherBrowserTest, TextRecognitionTest) {
  NavigateURL(https_server_.GetURL("a.com", "/image.html"));
  FetchPageContent("this is the way", false);
}

IN_PROC_BROWSER_TEST_F(PageContentFetcherBrowserTest,
                       TextRecognitionTestViaTextExtraction) {
  NavigateURL(https_server_.GetURL("twitter.com", "/image.html"));
  FetchPageContent("brave\nthis is the way", false);
}
