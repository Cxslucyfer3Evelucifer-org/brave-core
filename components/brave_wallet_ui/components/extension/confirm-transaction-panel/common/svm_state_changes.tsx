...
export const SPLTokenTransfer = ({
  ...
  const asset: IconAsset = React.useMemo(() => {
    const foundToken = combinedTokensList.find(
      (t) =>
        t.chainId === network.chainId && t.contractAddress === transfer.mint
    )
    ...
    return {
      ...
      symbol: transfer.symbol,
      chainId: network.chainId,
      isNft: METAPLEX_NFT_KINDS.includes(transfer.metaplexTokenStandard)
    }
...
export const SolStakingAuthChange = ({
  ...
  const hasStakerChange =
    authChange.currAuthorities.staker !== authChange.futureAuthorities.staker
  const hasWithdrawerChange =
    authChange.currAuthorities.withdrawer !==
    ...
    >
      {hasStakerChange ? (
        ...
        >
          ...
          <AddressChange
            fromAddress={authChange.currAuthorities.staker}
      ...
      {hasWithdrawerChange ? (
        ...
        >
          ...
          <AddressChange
            fromAddress={authChange.currAuthorities.withdrawer}
